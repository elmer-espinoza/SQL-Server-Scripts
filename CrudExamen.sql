...
create procedure Cliente_crear (
@nombres nchar(30),
@apellidos nchar(30),
@tipocliente nchar(30),
@situacionlaboral nchar(30),
@estado nchar(1))
as 
BEGIN
...
exec Cliente_crear 'nnn','aaa','tt','ss','1';

create procedure Cliente_desactivar (@clienteid int) as
begin
update cliente set estado = '0' where clienteid = @clienteid 
end 

exec Cliente_desactivar 3
...
AS
BEGIN
update cliente set nombres = @nombres, apellidos=@apellidos, tipocliente=@tipocliente, SituacionLaboral=@situacionlaboral, estado=@estado where Clienteid = @clienteid
END

exec Cliente_actualizar 3, 'NNNNN','AAAAA','TT','SS','2';

select * from cliente 

drop view Cliente_compras
create view Cliente_compras as select f1.clienteid, f1.nombres, f1.apellidos, f1.tipocliente, f1.situacionlaboral, f1.estado, coalesce(f2.nrocompras,0) nrocompras from cliente f1 left join compras f2 on f1.clienteid = f2.clienteid 
...
declare @TableName varchar(20) = 'Cliente'
declare @ClassName varchar(20) = 'Cliente'
